  ...
  try {
    ...
    const rows = await (
      ["admin","manager"].includes(me.role) && !userId
        ...
        : baseSelect
            .where(eq(requestsTable.userId, ["admin","manager"].includes(me.role) && userId ? parseInt(userId) : req.userId))
...
router.post("/", requireAuth, async (req: any, res) => {
  try {
    ...
    const [me] = await db.select().from(usersTable).where(eq(usersTable.id, req.userId)).limit(1);
    const targetId = ((me?.role === "admin" || me?.role === "manager") && body.targetUserId) ? body.targetUserId : req.userId;
...
router.patch("/:id", requireAdmin, async (req: any, res) => {
  try {
    ...
    if (existing.status !== "pending") return res.status(400).json({ error: "تمت المراجعة مسبقاً" });
    ...
    createNotificationForUser({
      ...
      type: "SYSTEM_ALERT",
      title: `${body.status === "approved" ? "✅" : "❌"} طلب ${typeLabels[existing.type] ?? existing.type} — ${body.status === "approved" ? "موافق عليه" : "مرفوض"}`,
      message: `تم ${statusAr} طلب ${typeLabels[existing.type] ?? existing.type} بتاريخ ${existing.date}.${body.adminNote ? ` ملاحظة: ${body.adminNote}` : ""}`,
      relatedId: id,
      relatedType: "request",
    }).catch(console.error);

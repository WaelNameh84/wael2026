...
router.get("/", requireAuth, async (req: any, res) => {
  try {
    ...
    const rows = await db.select({
      ...
      .where(["admin","manager"].includes(me.role) && !userId ? undefined : eq(bonusesTable.userId, targetUserId))
...
router.post("/", requireAdmin, async (req: any, res) => {
  try {
    ...
    createNotificationForUser({
      userId: body.userId,
      type: "SYSTEM_ALERT",
      title: `${body.type === "bonus" ? "🎉 مكافأة جديدة" : "⚠️ خصم من الراتب"}`,
      message: `تم إضافة ${typeAr} بمبلغ ${body.amount}${body.reason ? `. السبب: ${body.reason}` : "."}`,
      relatedId: record.id,
      relatedType: "bonus",
    }).catch(console.error);
...
router.get("/summary/:userId", requireAuth, async (req: any, res) => {
  try {
    ...
    const [me] = await db.select().from(usersTable).where(eq(usersTable.id, req.userId)).limit(1);
    if (!["admin", "manager"].includes(me.role) && targetId !== req.userId) {

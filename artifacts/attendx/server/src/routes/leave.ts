...
router.get("/:id", requireAuth, async (req: any, res) => {
  try {
    ...
    const r = rows[0];
    if (!["admin", "manager"].includes(me?.role ?? "") && r.userId !== req.userId) return res.status(403).json({ error: "Forbidden" });
...
router.patch("/:id", requireAuth, async (req: any, res) => {
  try {
    ...
    const schema = z.object({
      ...
      reason: z.string().optional(),
      /** Admin can explicitly set paid/unpaid regardless of leave type */
      isPaid: z.boolean().optional(),
    });
    const body = schema.parse(req.body);
    const [existing] = await db.select().from(leaveTable).where(eq(leaveTable.id, id)).limit(1);
    if (!existing) return res.status(404).json({ error: "Leave request not found" });
    if (!["admin", "manager"].includes(me.role) && existing.userId !== req.userId) {
      return res.status(403).json({ error: "Not allowed" });
    }
    const updates: any = {};
    if (body.status) updates.status = body.status;
    if (body.reason) updates.reason = body.reason;
    if (body.status && ["admin", "manager"].includes(me.role)) {
      updates.reviewedBy = req.userId;
      updates.reviewedAt = new Date();
    }
    // Admin/manager can override paid/unpaid at any time
    if (["admin", "manager"].includes(me.role) && body.isPaid !== undefined) {
    ...
    // Notify employee when admin/manager reviews their leave
    if (body.status && ["admin", "manager"].includes(me.role) && existing.userId !== req.userId) {
      const statusAr = body.status === "approved" ? "✅ موافق عليها" : body.status === "rejected" ? "❌ مرفوضة" : "ملغاة";
      createNotificationForUser({
        userId: existing.userId,
        type: "SYSTEM_ALERT",
        title: `إجازتك ${statusAr}`,
        message: `تم ${body.status === "approved" ? "الموافقة على" : body.status === "rejected" ? "رفض" : "إلغاء"} طلب إجازتك من ${existing.startDate} إلى ${existing.endDate}.`,
        relatedId: id,
        relatedType: "leave",
      }).catch(console.error);
    }

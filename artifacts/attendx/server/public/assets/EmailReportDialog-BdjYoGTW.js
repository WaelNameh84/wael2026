...
</html>`}function q(o){var a;const n=document.getElementById("__pdf_print_frame__");n&&n.remove();const s=document.createElement("iframe");s.id="__pdf_print_frame__",s.style.cssText="position:fixed;top:-10000px;left:-10000px;width:1px;height:1px;border:none;visibility:hidden;",document.body.appendChild(s);const t=s.contentDocument||((a=s.contentWindow)==null?void 0:a.document);if(!t){s.remove();return}t.open(),t.write(o),t.close();const r=()=>{var f,c;try{(f=s.contentWindow)==null||f.focus(),(c=s.contentWindow)==null||c.print()}catch{}setTimeout(()=>s.remove(),6e4)};t.readyState==="complete"?setTimeout(r,500):s.onload=()=>setTimeout(r,500)}function se(o){const n=j(o),s=new Blob([n],{type:"text/html;charset=utf-8"}),t=URL.createObjectURL(s),r=document.createElement("a");r.href=t,r.target="_blank",r.rel="noopener noreferrer",document.body.appendChild(r),r.click(),document.body.removeChild(r),setTimeout(()=>URL.revokeObjectURL(t),1e4)}function re(o){const n=j(o);q(n)}async function de(o,n,s){const t=j(s),r=await C(B("/api/reports/send-email"),{method:"POST",body:JSON.stringify({to:o,subject:n,html:t})});if(!r.ok){const a=await r.json().catch(()=>({}));throw new Error((a==null?void 0:a.error)||"Failed to send email")}}function le({open:o,onOpenChange:n,defaultEmail:s,isArabic:t,onSend:r}){const[a,f]=S.useState(s??""),[c,u]=S.useState(!1),[g,x]=S.useState("");S.useEffect(()=>{o&&(f(s??""),x(""))},[o,s]);const b=async()=>{const e=a.trim();if(!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(e)){x(t?"أدخل بريداً إلكترونياً صحيحاً":"Enter a valid email address");return}u(!0),x("");try{await r(e),n(!1)}catch(m){x((m==null?void 0:m.message)||(t?"فشل إرسال البريد":"Failed to send email"))}finally{u(!1)}};return l.jsx(F,{open:o,onOpenChange:e=>{c||n(e)},children:l.jsxs(U,{className:"max-w-sm",children:[l.jsx(W,{children:l.jsxs(M,{className:"flex items-center gap-2",children:[l.jsx(A,{className:"w-4 h-4"}),t?"إرسال التقرير عبر البريد":"Send Report by Email"]})}),l.jsxs("div",{className:"space-y-2 py-2",children:[l.jsx(J,{className:"text-xs text-muted-foreground",children:t?"البريد الإلكتروني للمستلم":"Recipient email"}),l.jsx(G,{type:"email",autoFocus:!0,value:a,onChange:e=>f(e.target.value),placeholder:"name@example.com",onKeyDown:e=>{e.key==="Enter"&&b()}}),g&&l.jsx("p",{className:"text-xs text-destructive",children:g})]}),l.jsxs(K,{children:[l.jsx(R,{variant:"outline",onClick:()=>n(!1),disabled:c,children:t?"إلغاء":"Cancel"}),l.jsxs(R,{onClick:b,disabled:c,className:"gap-2",children:[c?l.jsx(V,{className:"w-4 h-4 animate-spin"}):l.jsx(A,{className:"w-4 h-4"}),t?"إرسال":"Send"]})]})]})})}export{le as E,de as a,se as e,re as s};

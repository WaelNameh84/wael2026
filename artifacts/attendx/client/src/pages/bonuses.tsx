...
import { NoReportsIllustration } from "@/components/ui/empty-illustrations";
...
export default function BonusesPage() {
  ...
  const exportExcel = () => {
    const rows = filtered.map(b => ({
      ...
      السبب: b.reason ?? "",
      الفترة: b.period ?? "",
      التاريخ: b.createdAt ? new Date(b.createdAt).toLocaleDateString("ar-SY") : "",
    }));
      ...
      <div className="space-y-6 max-w-4xl">
        ...
        <div className="grid grid-cols-3 gap-4">
          <div className="bg-green-50 dark:bg-green-950/30 border border-green-200 dark:border-green-800 rounded-xl p-4 text-center">
            ...
            <p className="text-2xl font-bold text-green-600">+{formatCurrency(totalBonus, currency)}</p>
          </div>
          <div className="bg-red-50 dark:bg-red-950/30 border border-red-200 dark:border-red-800 rounded-xl p-4 text-center">
            ...
            <p className="text-2xl font-bold text-red-600">-{formatCurrency(totalDeduction, currency)}</p>
          </div>
          <div className={`border rounded-xl p-4 text-center ${net >= 0 ? "bg-blue-50 dark:bg-blue-950/30 border-blue-200 dark:border-blue-800" : "bg-orange-50 dark:bg-orange-950/30 border-orange-200 dark:border-orange-800"}`}>
            ...
            <p className={`text-2xl font-bold ${net >= 0 ? "text-blue-600" : "text-orange-600"}`}>{net >= 0 ? "+" : ""}{formatCurrency(Math.abs(net), currency)}</p>
          </div>
        </div>
        ...
        <div className="bg-card border border-card-border rounded-xl overflow-hidden">
          {isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center py-14 gap-3 text-center">
              ...
              <div>
                <p className="font-medium text-sm text-foreground/80">{isArabic ? "لا توجد سجلات" : "No records found"}</p>
                <p className="text-xs text-muted-foreground mt-1">{isArabic ? "ستظهر السجلات هنا عند إضافتها" : "Records will appear here once added"}</p>
              </div>
            </div>
          ) : (

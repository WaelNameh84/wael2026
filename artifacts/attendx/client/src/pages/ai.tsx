...
import AvatarAI, { type AvatarState } from "@/components/AvatarAI";
...
export default function AiPage() {
  ...
  const { ttsEnabled, wakeWord, assistantName, assistantPersonality, aiVoiceResponse } = useSettings();
  ...
  const sendMessage = useCallback(async (text: string, fromVoice = false) => {
    ...
    try {
      ...
      if (fromVoice || ttsEnabled || aiVoiceResponse) {
        setState("speaking");
  ...
  }, [isPending, speak, toast, ttsEnabled, aiVoiceResponse, assistantName, assistantPersonality, me?.name]);
          ...
          <div className="flex flex-col items-center py-6 mb-4 bg-card border border-border rounded-2xl relative">
            <AvatarAI
              state={
                ...
                state === "question-listening" ? "listening" :
                state === "wake-detected"      ? "happy"     :
                                                 "idle"
              }
              name={assistantName || "Aria"}
              message={undefined}
              size={160}
            />

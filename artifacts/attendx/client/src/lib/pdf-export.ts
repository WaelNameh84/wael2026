import { apiUrl, authFetch } from "./api-url";
...
export function buildReportHTML(opts: PdfReportOptions): string {
  ...
  if (payrollError) {
    ...
    const isDaily = payroll.contractType === "daily";
    const hoursWorked = payroll.totalNormalHours ?? 0;
    const baseSalaryDetail = isDaily
      ? `${fmt2(payroll.dailyRate)}/${L("يوم","day")} · ${fmt2(payroll.hourlyRate)}/${L("ساعة","hr")} · ${hoursWorked.toFixed(2)}h × ${fmt2(payroll.hourlyRate)}`
    ...
    // Bonus items: extra additions (admin bonuses, purchases)
    const extraBonuses = (payroll.bonusItems ?? []).filter(b => b.type === "bonus");
    extraBonuses.forEach((b, i) => {
      payRows.push([
        b.reason ?? L("مكافأة", "Bonus"),
        b.source === "purchase" ? L("مشتريات", "Purchase") : L("مكافأة إدارية", "Admin Bonus"),
        `+${fmt2(b.amount)}`,
        "#16a34a",
      ]);
    });

    // Bonus items: extra deductions (advances, admin deductions)
    const extraDeductions = (payroll.bonusItems ?? []).filter(b => b.type === "deduction");
    extraDeductions.forEach(d => {
      const isAdv = d.source === "advance";
      payRows.push([
        d.reason ?? (isAdv ? L("خصم سلفة", "Advance Deduction") : L("خصم إداري", "Admin Deduction")),
        isAdv ? L("سلفة", "Advance") : L("خصم إداري", "Admin"),
        `−${fmt2(d.amount)}`,
        "#dc2626",
      ]);
    });
...
/** @deprecated Opens a mailto: link — the native Mail app only supports plain text,
 * so the styled report design is lost. Use `emailReportHTML` instead. */
...
/**
 * Sends the same styled HTML used for the in-app PDF/print view as a real
 * email via the server, so the recipient sees the identical design instead
 * of the plain-text fallback that a `mailto:` link produces.
 */
export async function emailReportHTML(to: string, subject: string, opts: PdfReportOptions): Promise<void> {
  const html = buildReportHTML(opts);
  const res = await authFetch(apiUrl("/api/reports/send-email"), {
    method: "POST",

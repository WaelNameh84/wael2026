import { playNotifSound, getNotifSoundType } from "@/lib/alarm";
...
async function showNotification(title: string, body: string, icon = "/icon-192.svg") {
  ...
  // Play in-app notification sound (Web Audio — audible even when silent:true)
  try { playNotifSound(getNotifSoundType()); } catch {}

  // Prefer SW-backed notification (works in Chrome)
  if ("serviceWorker" in navigator) {
    try {
      const reg = await navigator.serviceWorker.ready;
      await reg.showNotification(title, { body, icon, badge: icon, vibrate: [200, 100, 200] } as NotificationOptions);
      return;
    } catch {
  ...
  // Legacy fallback
  try {
    new Notification(title, { body, icon } as NotificationOptions);
  } catch {
...
export function scheduleDailyReminders(
  lang: "en" | "ar" | "sv" = "en",
  startTime?: string,   // "HH:MM" — defaults to work-start from alarm settings or 07:00
  endTime?:   string,   // "HH:MM" — defaults to work-end   from alarm settings or 16:00
) {
  clearAllTimers();

  // Resolve times: prefer explicit args → alarm settings in localStorage → fallback
  const resolveTime = (arg: string | undefined, fallback: string): { h: number; m: number } => {
    const raw = arg ?? (() => {
      try {
        const s = JSON.parse(localStorage.getItem("setting_shift_alarms") ?? "{}");
        return fallback === "07:00" ? (s.startTime ?? fallback) : (s.endTime ?? fallback);
      } catch { return fallback; }
    })();
    const [h, m] = (raw as string).split(":").map(Number);
    return { h: isNaN(h) ? parseInt(fallback) : h, m: isNaN(m) ? 0 : m };
  };

  const start = resolveTime(startTime, "07:00");
  const end   = resolveTime(endTime,   "16:00");
  ...
  scheduleRepeating(start.h, start.m, m.morning.title, m.morning.body);
  scheduleRepeating(end.h,   end.m,   m.evening.title, m.evening.body);
}
...
export function bootNotifications(lang: "en" | "ar" | "sv" = "en", startTime?: string, endTime?: string) {
  if (!getDailyRemindersEnabled()) return;
  if (!("Notification" in window)) return;
  if (Notification.permission !== "granted") return;
  scheduleDailyReminders(lang, startTime, endTime);
}

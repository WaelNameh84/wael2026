...
/* ── 16. Pixel — 8-bit retro ─────────────────────────────────── */
function PixelClock({ time, date, size }: { time: string; date: string; size: ClockSize }) {
  const s = sizeMap[size];
  return (
    <div className={`flex flex-col items-center gap-1.5 ${s.pad} rounded-none`}
      style={{ background: "#1a1a2e", border: "3px solid #533483", imageRendering: "pixelated" }}>
      <div className={`font-mono font-bold tabular-nums tracking-widest ${s.time}`}
        style={{ color: "#e94560", fontFamily: "'Courier New', monospace", letterSpacing: "0.15em" }}>
        {time}
      </div>
      <div className="font-mono uppercase" style={{ color: "#0f3460", background: "#e0e0e0", padding: "1px 6px", fontSize: size === "large" ? 10 : 8 }}>
        {date}
      </div>
    </div>
  );
...
/* ── 17. Sunburst — radial rays analog ────────────────────────── */
    ...
    <div className="flex flex-col items-center gap-1.5">
      <svg width={dim} height={dim} viewBox={`0 0 ${dim} ${dim}`} className="select-none">
        ...
        {rays.map(i => {
          ...
          return <line key={i}
            x1={pt(a, r * 0.62).x} y1={pt(a, r * 0.62).y}
            x2={pt(a, r).x} y2={pt(a, r).y}
            stroke="#f59e0b" strokeOpacity={i % 2 === 0 ? 0.8 : 0.4} strokeWidth={i % 2 === 0 ? 3 : 1.5} strokeLinecap="round" />;
        })}
        <circle cx={cx} cy={cx} r={r * 0.6} fill="url(#sunGrad)" stroke="#d97706" strokeWidth="1.5" />
        <line x1={cx} y1={cx} x2={pt(hrA, r * 0.3).x} y2={pt(hrA, r * 0.3).y} stroke="#7c2d12" strokeWidth={dim < 130 ? 3 : 4} strokeLinecap="round" />
        <line x1={cx} y1={cx} x2={pt(minA, r * 0.45).x} y2={pt(minA, r * 0.45).y} stroke="#7c2d12" strokeWidth={dim < 130 ? 2 : 3} strokeLinecap="round" />
        <circle cx={cx} cy={cx} r={3} fill="#7c2d12" />
      </svg>
      <span className={`text-muted-foreground ${sizeMap[size].date}`}>{date}</span>
    </div>
  );
}

/* ── 18. Holographic — iridescent shimmer ─────────────────────── */
function HolographicClock({ time, date, size }: { time: string; date: string; size: ClockSize }) {
  ...
  return (
    <div className={`flex flex-col items-center gap-1.5 ${s.pad} rounded-2xl relative overflow-hidden`}
      ...
      <div className="absolute inset-0 pointer-events-none" style={{
        background: "linear-gradient(120deg, #ff9a9e, #fecfef, #a1c4fd, #c2e9fb, #ff9a9e)",
        backgroundSize: "400% 400%",
        opacity: 0.25,
        animation: "holoShift 6s ease infinite",
      }} />
      <style>{`@keyframes holoShift { 0%{background-position:0% 50%} 50%{background-position:100% 50%} 100%{background-position:0% 50%} }`}</style>
      <div className={`font-mono font-bold tabular-nums tracking-widest relative z-10 ${s.time}`}
        style={{
          background: "linear-gradient(120deg, #ff9a9e, #a1c4fd, #c2e9fb, #ff9a9e)",
          backgroundSize: "300% auto",
          WebkitBackgroundClip: "text",
          WebkitTextFillColor: "transparent",

...
export default function NotificationsPanel({ onClose }: { onClose: () => void }) {
  ...
  const KNOWN_DETAIL_TYPES = ["leave", "bonus", "work_report"];
        ...
        <DialogContent className="max-w-sm max-h-[90vh] overflow-y-auto">
          ...
          {selectedNotif && (
            <div className="space-y-3 pb-1">
              ...
              {/* ═══ DEFAULT / GENERIC (attendance, late_justification, user, request, salary_advance, or unknown) ═══ */}
              {!KNOWN_DETAIL_TYPES.includes(selectedNotif.relatedType) && (
                <>
                  <div className="flex items-center gap-2 flex-wrap">
                  ...
                  <div className="bg-muted/40 rounded-lg px-4 py-3">
                    <p className="text-sm font-medium leading-relaxed">{selectedNotif.title}</p>
                  </div>
                  {selectedNotif.message && (
                    <div className="bg-muted/20 border border-border rounded-lg px-4 py-3">
                      <p className="text-sm text-foreground/90 leading-relaxed whitespace-pre-wrap">{selectedNotif.message}</p>
                    </div>
                  )}
                  ...
                  {loadingRelated ? (
                    ...
                    <div className="bg-muted/20 border border-border rounded-lg px-4 py-3 space-y-1.5">
                      {relatedData.userName && (
                        <div className="flex justify-between text-xs">
                          <span className="text-muted-foreground">{t("employee") || "الموظف"}</span>
                          <span className="font-medium">{relatedData.userName}</span>
                        </div>
                      )}
                      {relatedData.name && !relatedData.userName && (
                        <div className="flex justify-between text-xs">
                          <span className="text-muted-foreground">{t("employee") || "الموظف"}</span>
                          <span className="font-medium">{relatedData.name}</span>
                        </div>
                      )}
                      {relatedData.email && (
                        <div className="flex justify-between text-xs">
                          <span className="text-muted-foreground">Email</span>
                          <span className="font-medium">{relatedData.email}</span>
                        </div>
                      )}
                      {relatedData.date && (
                        <div className="flex justify-between text-xs">
                          <span className="text-muted-foreground">{t("date") || "التاريخ"}</span>
                          <span className="font-medium">{relatedData.date}</span>
                        </div>
                      )}
                      {relatedData.checkIn && (
                        <div className="flex justify-between text-xs">
                          <span className="text-muted-foreground">{t("check_in") || "تسجيل الدخول"}</span>
                          <span className="font-medium">{new Date(relatedData.checkIn).toLocaleTimeString()}</span>
                        </div>
                      )}
                      {relatedData.status && (
                        <div className="flex justify-between text-xs">
                          <span className="text-muted-foreground">{t("status") || "الحالة"}</span>
                          <span className="font-medium">{relatedData.status}</span>
                        </div>
                      )}
                      {relatedData.locationName && (
                        <div className="flex justify-between text-xs">
                          <span className="text-muted-foreground">{t("location") || "الموقع"}</span>
                          <span className="font-medium">{relatedData.locationName}</span>
                        </div>
                      )}
                      {relatedData.reason && (
                        <div className="pt-1 border-t border-border/50">
                          <p className="text-xs text-muted-foreground mb-0.5">{t("reason") || "السبب"}</p>
